...
The core idea: six coloured **voices**, each carrying rhythms whose hits are spread
...
The track view shows each voice as a **horizontal row**, time running left to right at a
fixed **16 bars per screen width** (`BARS_PER_SCREEN`) inside a horizontal scroller. A long
track is genuinely long and you scroll along it — an earlier design squeezed the whole track
into one screenful and went unreadable past a few dozen bars. Where you have scrolled to is
**where playback starts**: the bar at the left edge is the bar ▶ begins on, marked by the
start line welded to that edge and named in the top bar ("bar 17"). While playing, the
scroller follows the playhead until you scroll by hand; ■ leaves the view on the bar it
stopped at, so ▶ resumes there. The lane carries a screenful of empty tail so the last bars
can reach the left edge and be started from.

There are three views, and only three: the **track**, one voice's **grid**, and the
**mixer**. Tapping a row opens that voice's **all-loops grid** — the bar-square placement
grid with a loop picker, every loop of the colour showing at once — landing on the loop
under your finger when the bar is painted (the paint is hit-tested per bar, and each loop of
a colour has its own shade), on its first loop when the bar is bare. A loop's sound is
reached from there. The ▦ heads to the left of the scroller open the same grid on loop 1.
A loop's page is the whole loop, top to bottom:
...
There is no loop LIST any more — picking a loop happens on the row
...
- **Voice / colour / line:** there are `NUM_LINES = 6` voices (`src/model/lines.ts`), one per
  overview row and per letter of the logo, each with an identity colour in
  `VOICE_COLORS`. All six hold loops.
...
- **Loop** (`src/model/track.ts`): a sound + rhythm + a **placement rule** (`EveryRule`:
  weight / dice / every-Nth / power-of-2 / explicit bars / fill) and a sounding length in
  bars. Rules are seeded so weighted rolls are reproducible.
- **Track & compile** (`src/model/track.ts`): the authoring surface is a `Track` (bar limit +
  ...
  `VoiceNode` chains ("lanes") that everything downstream (engine, WAV export, the overview)
  runs on. A colour compiles to **exactly one lane**: its loops never stack simultaneously,
  they resolve by list priority (earliest wins the bar) in `resolveLane`, which is what lets
  one overview row stand for a whole voice — a bar of a colour has exactly one loop sounding,
  so the row is a single per-bar array (`colorBarNumbers`) painted as one gradient.
