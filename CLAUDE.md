...
The track view shows each voice as a **vertical column of 16 blocks**: a block covers
...
- **Voice / colour / line:** there are `NUM_LINES = 6` voices (`src/model/lines.ts`), one per
  overview column and per letter of the logo, each with an identity colour in
...
- **Track & compile** (`src/model/track.ts`): the authoring surface is a `Track` (bar limit +
  ...
  one narrow overview column stand for a whole voice.
